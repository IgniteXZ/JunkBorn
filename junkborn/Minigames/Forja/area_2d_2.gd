...
func _abrir_forja():
	if canvass:
		...
		# TRUQUE VISUAL: Move a tela de forja para ser o primeiro nó a ser desenhado no Canvas.
		# Isso joga ela para trás do inventário e do preview de arrasto do mouse!
...
func _on_area_entered(area: Area2D) -> void:
	...
	# BUSCA DINÂMICA: Pegamos a Label de dentro do próprio nó Interagir do player que colidiu!
	label_interagir_do_player = area.get_node_or_null("LabelInteragir")
	if not label_interagir_do_player:
		# Se não achou na raiz do player, procura dentro do nó "Interagir"
